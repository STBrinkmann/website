...
Exposure to residential greenness or green spaces such as parks or gardens are beneficial for multiple measures of health [@markevych2017; @labib2020]. Both of these terms will be used as synonyms henceforth. Greenspace exposure can be categorised into three types: (a) availability, refering to the physical amount of greenspace, (b) accessibility, meaning the spatial proximity to greenspace, and (c) visibility, refering to the visual perception of greenness [@labib2020].

In [our](https://www.geography.nat.fau.eu/research/cultural-geography/wg-digital-health/#collapse_5) recent pblication (in submission) we measured greenness taking a top-down, bird's eye view approach using remote sensing derived [NDVI](https://eos.com/ndvi/), to aproximate the availability of greenness. Furthermore, we used a distance weighted road network to calculate potential neighbourhood exposure models around participants place of residency, therefore also accounting for accessibility.

The next step will be, to combine greenness visibilty with our potential neighbourhood exposure models.

In this post, I will therefore demonstrate how to download and prepare all necessary files and methods needed for a visibility analysis for Vancouver, Canada. In the first part I demonstrate the data acquisition, in the second part I will explain the main functions used for the visibility analysis. My implementation of these methods is very light weighted and fast, while still maintaing high resolution.

## Libraries

First load all packages. If one of these packages has not been installed, use the `install.packages()` function.

[terra](https://CRAN.R-project.org/package=terra) is a relativly new R package that replaces the well known [raster](https://CRAN.R-project.org/package=raster). I have found `terra` to work much faster for most tasks. However, in the visibility-functions, using `raster` proved to work much faster.
...
The data is beeing stored in a different directory than this R project. Therefore I first need to assign my working directory.
...
To calculate the visibility of a point B from the point A, we first need to access all raster cells from point A to B. The `rasterprofile` function returns a `data.table` with all cells from A to B, containing X- and Y-coordinates, the height and cell-number for every cell. Using the `data.table` structure as the core fr the following functions, significantly reduced computation time.
...
The observer at point A can only see point B, if no object in between point A and B blocks the view to point B. The `lineOfSight` function evaluates visibility along all cells from A to B, by calculating tangent *⍺* from *𝚫height (*opposite side) and *distance traveled* (adjacent side) and comparing it for every step. The function returns a `data.frame` containing the cell number and corresponding visibility of all points from A to B. Visible cells have a value of 1 and non visible cells -1.
...
lineOfSight <- function(xy1, x0, y0, 
  ...
  # How far we have travelled along the ray
  ...
  # Set starting point to +1 and initilize max_dydx
  max_dydx <- -Inf
  ...
  for(i in 2:nrow(pixels)) {
    ...
    cur_dydx = (pixels[[3]][i] - height0) / (distance_traveled * resolution)
  ...
  non_visible_pixels <- pixels[[4]][!pixels[[4]] %in% visible_pixels]
...
viewshed <- function(sf_start, max_distance, dsm_data, dtm_data, resolution, 
  ...
  # resample the DSM and DEM to the lower resolution.
  ...
  while (repeating) {
    ...
    # Apply lineOfSight function on every point in sf_stop_i
    if (cores > 1) {
      if (Sys.info()[["sysname"]] == "Windows") {
        cl <- parallel::makeCluster(cores)
...
The animation below illustrates, the functionality of the `viewshed` function. Starting with a raster of unknow visibilty (yellow), we iterativly call the `lineOfSight` function and set the status of each raster cell to visible (green) or no-visible (white).
...
Runtime: \~13 seconds\
Total visibility: 9.4%\
Visible area: 33 670 m²
...
Runtime: \~6 seconds\
Total visibility: 9.7%\
Visible area: 34 984 m²
...
Runtime: \~3 seconds\
Total visibility: 10.2%\
Visible area: 36 680 m²
...
Runtime: \~2 seconds\
Total visibility: 11.3%\
Visible area: 40 800 m²
...
The `visibleGreen` function returns the proportion of visible greenspace of total visibility.
...
A mean value of 0.47 indicates, that almost 50% of the visible area along the network is vegetated. However, we can see a high diversity in the example above, in that the northern half has higher visibly greenness. Therefore, future research may also incorporate actual activity space meassurements instead of potential activity space, to further analyse where participants spend time and thus improve the understanding of greenspace related health effects.

## Conclusion

Data acquisition and processing with `lidR`and `terra` is simple and fast using R as the only tool. The implementation of a parallel viewshed algorithm has proven to be very light-weighted and fast. Using a lower resolution significantly reduces model runtime. However, even at highest resolution, the runtime is acceptable. The effect of multiprocessing is significant only with high resolution or very large values of `max_distance`. Using this algorithm in a large scale study at high resolution appears to be practical. Compared to the model 1, model 4 has an increase of 20% in visible area. In my opinion the trade-off between loss of accuracy for an increase in speed still is acceptable. I would suggest comparing a few points with 1m, 2m and 5m resolution.

---
title: 'Visible Greenness Visibility Exposure'
...
lineOfSight <- function(xy1, x0, y0, height0, resolution, dsm_data) {
  ...
  data.table::as.data.table(matrix(pixels[, 4:5], ncol = 2))
}
```

### Viewshed

Finally, the visibility of all points withing a certain buffer around point A can be calculated, using the `lineOfSight` function. The `viewshed` function returns a circular raster (start point + max_distance-buffer) where values of 1 indicate visible points and -1 non-visible points.
...
Runtime: 1.5 seconds (cores=1: 4.8 seconds)\
...
Runtime: 0.9 seconds (cores=1: 1.6 seconds)\
...
Runtime: 0.5 seconds (cores=1: \~0.6 seconds)\

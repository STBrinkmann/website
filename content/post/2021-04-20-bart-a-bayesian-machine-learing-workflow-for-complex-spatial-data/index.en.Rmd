...
In this post I will use the data we have used in our COVID-19 paper, to analyze age-adjusted incidence rates of COVID-19 for German counties. Even though the BART model has great predictive power, we will not predict new cases of COVID-19, but rather use it as a exploratory tool to understand, what factors contribute to the spreading of COVID-19.
...
Let's double check if the correct amount of RAM has been made available. If the message states a completely different amount of memory, try to manually write the java.parameters string, instead of copy&pasting.
...
The Pseudo-R² of 0.92 and RMSE of 0.20 look quiet promising, but let's check for error assumptions.
...
The `var_selection_by_permute` function performs variable selection introduced by @bleich2014, to reduce model complexity. Let's run the next chunk and investigate the most important variables. You may skip this step just use my results, as this will take quiet long to run...
...
Spatial autocorrelation can be tested by mapping the residuals. I have provided a cleaned shapefile of the NUTS3 (federal states) regions in Germany. The mapped residuals allow us to visualize under- and overpredictions and check for spatial clustering of residuals. To determine whether the spatial clustering of residuals is statistically significant or not, one could [compute Moran's I](https://mgimond.github.io/Spatial/spatial-autocorrelation-in-r.html#app8_3). But since there is no discernible pattern of autocorrelation - the residuals are randomly distributed -, I will skip this step.

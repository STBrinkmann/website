...
workdir <- "/media/sebastian/Red/Vancouver/Vancouver_Sample_Data/"
...
When looking at the VTVI layer, higher values are rarely achieved (4th quantile = ) because it is generally harder to view trees compared to overall vegetation. The two big parks in the north and south are the areas with the highest VTVI values. The residential area in the east also achieved relatively high values. This might be explained by the very large London plane trees (*Platanus × acerifolia*), European beech trees (*Fagus sylvatica*) and horse chestnut trees (Aesculus hippocastanum) that have been planted in the side alleys.
